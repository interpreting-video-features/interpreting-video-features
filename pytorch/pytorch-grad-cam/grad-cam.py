...
class FeatureExtractor():
    ...
    def __call__(self, x):
        ...
        for name, module in self.model._modules.items():
            #print("looking at name module name: ", name)
            
            #logits layer doesn't have the right shape, skip it
            if(self.archType=="I3D" and (name=='logits' or name=='dropout' or name=='avg_pool' or name=='sm')):
                continue
            
            if(self.archType=="CLSTM" and name=="endFC" and self.model.UseEntireSeq==False):
                ...
                #print("gradcam x shape: ", x.shape)
                x = module(x)
            if name in self.target_layers:
                if(self.archType=="CLSTM" and name=="firstCLSTMLayer" and self.model.UseEntireSeq==False):
                    aggOut = Variable(torch.stack(x[0]), requires_grad=True)
                    aggOut.register_hook(self.save_gradient)
                    outputs += [aggOut]#[x[0][-1]]
                if(self.archType=="CLSTM" and name=="firstCLSTMLayer"):
                    ...
                    outputs += [x]
                #print("inside inside shape for ", name," :", x.shape)
        return outputs, x
