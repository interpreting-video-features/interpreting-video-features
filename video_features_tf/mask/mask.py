...
def perturb_sequence(seq, mask, perb_type='freeze', snap_values=False):
    ...
    if (perb_type == 'reverse'):
        ...
        subMasks = find_submasks_from_mask(mask)
        ...
        for mask_on_inds in subMasks:
            ...
            for u in range(int(len(mask_on_inds) // 2)):
                ...
                perb_input[:, mask_on_inds[u], :, :, :] = (1 - mask[mask_on_inds[u]]) * seq[:, mask_on_inds[u], :, :, :] + \
                                                       mask[mask_on_inds[u]] * seq[:, mask_on_inds[-(u + 1)], :, :, :]
                perb_input[:, mask_on_inds[-(u + 1)], :, :, :] = (1 - mask[mask_on_inds[u]]) * seq[:, mask_on_inds[-(u + 1)],
                                                                                          :, :, :] + mask[mask_on_inds[
                    u]] * temp
    return perb_input
